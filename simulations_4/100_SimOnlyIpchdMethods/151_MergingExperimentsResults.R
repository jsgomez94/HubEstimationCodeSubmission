...
rm(list = ls())
load("experiments4/data/output5.RData")
dim(output5)
rm(list = ls())



###################### Merging BIC Results for p = 100:
## We have results split in 4 files.
runtype <- 2
index   <- 1
for (sim_ind in 1:288) {
  ...
  ## Create directory where results will be saved:
  runtype_name    <- c("pretraining", "experiments", "outputs")[runtype]
  results_dir     <- paste0(runtype_name, index, "/bic_data/")
  
  load(paste0(results_dir, "output_bic", sim_ind, "_0.RData"))
  output_0 <- get(paste0("output_bic", sim_ind, "_0"))
  args_0   <- get(paste0("args", sim_ind))
  n_sim <- args_0$nsim
  for (microrun_ind in 1:9) {
    load(paste0(results_dir, "output_bic", sim_ind, "_", microrun_ind,".RData"))
    output_0 <- get(paste0("output_bic", sim_ind, "_", microrun_ind)) %>%
      mutate(sim = sim + microrun_ind * n_sim) %>%
      {bind_rows(output_0, .)}
  ...
  assign(
    ...
    args_0,
    pos = tmp_env_sim)
  print(paste0(results_dir, "output_bic", sim_ind, ".RData"))
  save(
    list = ls(all.names = TRUE, pos = tmp_env_sim), envir = tmp_env_sim,
    file = paste0(results_dir, "output_bic", sim_ind, ".RData"))
...
rm(list = ls())
load("experiments4/bic_data/output_bic5.RData")
dim(output_bic5)
rm(list = ls())
